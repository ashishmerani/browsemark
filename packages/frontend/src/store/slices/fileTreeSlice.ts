...
const LOCAL_STORAGE_KEY_PREFIX = 'browsemark_expanded_nodes_';
const LOCAL_STORAGE_RECENT_PATHS_KEY = 'browsemark_recent_paths';
const MAX_RECENT_PATHS = 10;

// Migrate legacy localStorage keys on first load
const LEGACY_PREFIX = 'markdown_vault_expanded_nodes_';
const LEGACY_RECENT_KEY = 'markdown_vault_recent_paths';
(() => {
  try {
    if (!localStorage.getItem(LOCAL_STORAGE_RECENT_PATHS_KEY) && localStorage.getItem(LEGACY_RECENT_KEY)) {
      const recentPaths = localStorage.getItem(LEGACY_RECENT_KEY);
      if (recentPaths) {
        localStorage.setItem(LOCAL_STORAGE_RECENT_PATHS_KEY, recentPaths);
        const paths: string[] = JSON.parse(recentPaths);
        for (const p of paths) {
          const oldVal = localStorage.getItem(`${LEGACY_PREFIX}${p}`);
          if (oldVal) {
            localStorage.setItem(`${LOCAL_STORAGE_KEY_PREFIX}${p}`, oldVal);
