...
import { toggleFileTree, toggleOutline, toggleContentMode } from './store/slices/appSettingSlice';
...
const Layout: React.FC<LayoutProps> = ({ onSettingsClick }) => {
  ...
  const { fileTreeOpen, outlineOpen, contentMode } = useSelector((state: RootState) => state.appSetting);
  ...
  const handleToggleContentMode = useCallback(() => {
