...
vi.mock('../../../../src/store/slices/appSettingSlice', async () => ({
  ...
  saveAppSetting: vi.fn(() => ({ type: 'test/appSetting/saveAppSetting' })),
}));

vi.mock('../../../../src/store/slices/configSlice', async () => ({
  __esModule: true,
  ...(await vi.importActual('../../../../src/store/slices/configSlice')),
  saveConfigToBackend: vi.fn(() => ({ type: 'test/config/saveConfigToBackend' })),
}));
...
describe('SettingsDialog', () => {
  ...
  test('dispatches saveAppSetting and saveConfigToBackend on Save button click', async () => {
    ...
    await waitFor(() => {
      ...
      expect(saveConfigToBackend).toHaveBeenCalledWith({ theme: 'default', fontFamily: 'Inter', fontFamilyMonospace: 'JetBrains Mono', fontSize: 14, syntaxHighlighterTheme: 'auto' });
  ...
  test('resets to default settings on Reset to Default button click', async () => {
    ...
    await waitFor(() => {
      ...
      expect(saveConfigToBackend).toHaveBeenCalledWith({ fontFamily: 'Inter', fontFamilyMonospace: 'JetBrains Mono', fontSize: 14, theme: 'default', syntaxHighlighterTheme: 'auto' });
    });
  });
  ...
  test('changes font family via select', async () => {
    ...
    const select = await waitFor(() => screen.getByTestId('font-family-select'));
  ...
  test('changes monospace font family via select', async () => {
    ...
    const select = await waitFor(() => screen.getByTestId('monospace-font-family-select'));
    const button = within(select).getByRole('combobox');
    fireEvent.mouseDown(button);
