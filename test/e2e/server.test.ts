...
describe('Server E2E Tests', () => {
  ...
  it('GET /api/markdown/browsemark-welcome.md should return the welcome markdown', async () => {
    const res = await request(app).get('/api/markdown/browsemark-welcome.md');
    expect(res.statusCode).toEqual(200);
    expect(res.text).toContain('# browsemark');
  });
  ...
  it('GET / should return index.html for root path', async () => {
    ...
    expect(res.statusCode).toEqual(200);
    expect(res.text).toContain('<title>browsemark</title>');
  });
  ...
  it('should prevent path traversal on /api/outline', async () => {
    const res = await request(app).get('/api/outline?filePath=../../package.json');
