...
describe('config', () => {
  ...
  const configPath = path.join(os.homedir(), '.config', 'browsemark', 'config.json');
  ...
  it('saveConfig should merge new config with existing config before writing', () => {
    (fs.existsSync as Mock).mockReturnValue(true);
    (fs.readFileSync as Mock).mockReturnValue(JSON.stringify({ fontFamily: 'ExistingFont', fontSize: 18 }));
