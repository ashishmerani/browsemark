...
describe('cli', () => {
  ...
  it('announcement URL matches package.json repository', async () => {
    setExistsSyncResult(false);
    process.argv = ['node', 'cli.ts', '.'];

    const realFs = await vi.importActual<typeof import('fs')>('fs');
    const realPkg = realFs.readFileSync(
      path.join(__dirname, '../../package.json'), 'utf8'
    );
    const repoUrl = JSON.parse(realPkg as string).repository.url
      .replace('git+', '').replace('.git', '');

    return cli.run()
      .then(() => {
        const mockLog = logger.log as Mock;
        const announcementCalls = mockLog.mock.calls
          .filter(([tag]: [string]) => tag === 'Announcement');
        const urlCall = announcementCalls.find(
          ([, msg]: [string, string]) => msg.includes('github.com')
        );
        expect(urlCall).toBeDefined();
        expect(urlCall![1]).toContain(repoUrl);
      });
  });
});
